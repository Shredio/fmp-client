...
final class BalanceSheetStatementTest extends TestCase
{
	...
	{
		$client = $this->createClient(__DIR__ . '/fixtures/balance-sheet-statement-bulk-nan.csv');

		$statements = iterator_to_array($client->balanceSheetStatementBulk(2024));
		...
		$this->assertSame((new BalanceSheetStatement(
			...
			minorityInterest: 0,
			totalLiabilitiesAndTotalEquity: 100000000,
			totalInvestments: 0,
			totalDebt: 0,
			netDebt: 0,
		))->toArray(), $statements[0]->toArray());
		...
		$this->assertSame((new BalanceSheetStatement(
			...
			minorityInterest: 0,
			totalLiabilitiesAndTotalEquity: 300000000,
			totalInvestments: 0,
			totalDebt: 0,
			netDebt: 0,
